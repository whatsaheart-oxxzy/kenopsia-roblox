...
| `podium` | MachineFlow (23.08.2026, P1.3) | `{cam={pos={x,y,z}, look={x,y,z}}, rows={{slot, place, userId, displayName, score}…}, seconds}` after the 6 s verdict stamp, to `room.members` AND `room.spectators`; the bare `{kind="podium"}` 8 s later is the clear. Client: `hideAll()`, fixed camera via the RenderStepped writer, phosphor caption. Shapes in `Shared/Rules/MachineCam.luau`. |
| `audiencecam` | MachineFlow (23.08.2026, P1.7) | `{cf={pos={x,y,z}, look={x,y,z}}, trialId}` to `room.spectators` (the bench) when the Trial stage begins and to a late joiner seated mid-trial; the bare `{kind="audiencecam"}` at trial end / session cleanup is the clear. Eye per trial resolved live by `Services/AudienceCam.luau` (canteen `Rig.ObserverCamera`, Dead Zone above `Cameraplacement`, Bird Hunting above the start wall, kit arenas from their origin). Never sent to participants. |
